import os
import sys
sys.path.append("./../argoverse-api/argoverse")
from evaluation import eval_tracking as et
import argparse
import time
import glob
import json
...
parser.add_argument('--fname', type=str, default="val_t21_a3_h3_ioudot1", help='detection threshold')
parser.add_argument('--set', type=str, default="val", help='val/test/sample')
...
op_path = os.path.join("./argo_results", args.fname  )
dr = os.listdir(op_path)
dr.sort()
op_lst = []
for files in dr:
    op_lst.append(os.path.join(op_path,files,"per_sweep_annotations_amodal/"))

op_lst.sort()    

d_path = "./../argodataset/argoverse-tracking/" + args.set + "/"

ldr = os.listdir(d_path)
d_lst = []
for files in ldr:
    d_lst.append(d_path+files)

d_lst.sort()

print(op_lst[:3],d_lst[:3])

out_filename = "./argo_eval/" + args.fname + ".txt"


with open(out_filename, "w") as out_file:
    eval_tracks(
        op_lst, d_lst ,0,100, out_file, "average")
